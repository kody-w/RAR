...
__manifest__ = {
    ...
    "dependencies": [
        "@rapp/basic_agent",
        "@kody-w/copilot_studio_deploy_agent",
    ],
    "example_call": {
        "args": {
            "action": "plan",
...
_DEPLOYER_MODULES = (
    ...
    "agents.rar_kody_w_copilot_studio_deploy_agent",
    "copilot_studio_deploy_agent",
    "rar_kody_w_copilot_studio_deploy_agent",
)
...
def _load_deployer():
    ...
    raise RuntimeError(
        ...
        "@kody-w/copilot_studio_deploy_agent dependency. "
        + "; ".join(failures[-2:])
    )
...
class RappCopilotStudioFactoryAgent(BasicAgent):
    ...
    def _extend(self, **kwargs):
        ...
        display_name = str(
            kwargs.get("display_name") or old_manifest.get("display_name") or ""
        ).strip()
        environment = str(
            kwargs.get("environment") or old_manifest.get("environment") or ""
        ).strip()
        prefix = str(
            kwargs.get("publisher_prefix")
            or old_manifest.get("publisher_prefix")
            or ""
        ).strip()
