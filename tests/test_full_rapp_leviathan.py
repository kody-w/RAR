...
def test_assessment_requires_evidence_for_every_organ_and_plane(tmp_path):
    ...
    architecture["planes"]["evidence-recovery"]["evidence"] = {}
    stable = {
        key: value
        for key, value in architecture.items()
        if key != "sha256"
    ...
    architecture["sha256"] = module._digest(stable)
    partial = module.assess(architecture, [trust_anchor()])
    assert partial["full_leviathan"] is False
    assert "evidence-recovery" in partial["missing_planes"]
